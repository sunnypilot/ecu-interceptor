...
// static void hyundai_canfd_adas_drv_interceptor_rx_hook(CANPacket_t *to_push) {
//   UNUSED(to_push);
//  
// }

static void hyundai_canfd_adas_drv_interceptor_tamper_hook(CANPacket_t * to_send) {
...
static bool hyundai_canfd_adas_drv_interceptor_fwd_hook(int bus_num, int addr) {
  UNUSED(hyundai_canfd_adas_drv_interceptor_tamper_hook);
  const int is_scc_msg = addr == 0x1A0;
  ...
  block_adas_drv_ecu = block_adas_drv_ecu && is_scc_msg && (bus_num == ADAS_DRV_BUS || bus_dst == ADAS_DRV_BUS);
  block_adas_drv_ecu = true;
  return false;
}
